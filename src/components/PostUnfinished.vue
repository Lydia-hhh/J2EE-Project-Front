...
<!--          <el-table-column label="订单编号" prop="orderNum"></el-table-column>-->
          ...
          <el-table-column label="操作">
            <template #default="scope">
              ...
              <el-button v-if="scope.row.packageState==='已分配'" type="danger" size="mini" @click="editState(scope.row)">确认</el-button>
              <el-button v-if="(scope.row.packageState==='已送到' || scope.row.packageState==='已完成')" type="success" circle size="mini"><el-icon><check/></el-icon></el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
  ...
  methods:{
    ...
    editState(row){
     ...
     this.newRow.id=this.row.id;
      request.put('postman/orderundo',this.newRow).then(res => {
        console.log(res);
        if(res.code==='0'){
          this.$message({
            type:'success',
